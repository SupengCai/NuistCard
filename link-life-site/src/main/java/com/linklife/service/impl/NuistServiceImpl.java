...
public class NuistServiceImpl {
	...
	public JSONObject login( NuistLoginModel nuistLoginModel, HttpServletRequest request, HttpServletResponse response ) {
		...
		JSONObject jsonObject = NuistCardAPI.nuistPost4JSON( nuistLoginModel.getUsernumber(), NuistCardAPI.NUIST_CARD_LOGIN_API,
		...
		if( jsonObject.isNull( NuistCardAPI.REQUEST_STATE ) ) {
			...
			if( nuistLoginModleLocal.getPassword().equals( nuistLoginModel.getPassword() ) ) {
				...
				if( nuistLoginModel.getRememberme() ) {
					CookieTool.addCookie( response, CookieTool.COOKIE_USER_ACCOUNT, nuistLoginModel.getUsernumber(), 60 * 60 * 24 * 7 );
					CookieTool.addCookie( response, CookieTool.COOKIE_USER_PWD, Base64.Decode( nuistLoginModel.getPassword() ), 60 * 60 * 24 * 7 );
		...
		// 判断用户登陆状态
			...
			NuistLoginHistoryModel nuistLoginHistoryModel = new NuistLoginHistoryModel();
			nuistLoginHistoryModel.mockRepository( nuist.getNuistLoginHistoryModel().acquiredTargetRepository() );
			nuistLoginHistoryModel.insertLoginHistory( accountId, remoteAddr, RegexUtil.explorerFilter( request.getHeader( "User-Agent" ).toString() ) );
	...
	public String getNuistScoreJson( String pageIndex, HttpServletRequest request, HttpServletResponse response ) {
		...
		JSONObject jsonObject = NuistCardAPI
				.nuistPost4JSON( userAccount, NuistCardAPI.NUIST_CARD_MOBILE_SCORE, getNuistMobileScorePostParams( obj, pageIndex ) );
	...
	public boolean moblieLogin( NuistLoginModel nuistLoginModel ) {
		...
		try {
			jsonObject = NuistCardAPI.nuistPost4JSON( nuistLoginModel.getUsernumber(), NuistCardAPI.NUIST_CARD_MOBILE_LOGIN,
					getNuistMobileLoginPostParams( nuistLoginModel ) );
	...
	public String moblieLogin( NuistLoginModel nuistLoginModel, HttpServletResponse response ) {
		...
		JSONObject jsonObject = NuistCardAPI.nuistPost4JSON( nuistLoginModel.getUsernumber(), NuistCardAPI.NUIST_CARD_MOBILE_LOGIN,
				getNuistMobileLoginPostParams( nuistLoginModel ) );
	...
	public JSONObject recharge( NuistLogin nuistLogin, RechargeModel rechargeModel, HttpServletRequest request ) {
		...
		// 登陆成功进行充值
		JSONObject jsonObject = NuistCardAPI.nuistPost4JSON( nuistLogin.getUsernumber(), NuistCardAPI.NUIST_CARD_RECHARGE_API,
				getNuistRechargePostParams( rechargeModel ) );
		if( jsonObject.getBoolean( NuistCardAPI.REQUEST_STATE ) ) {
			// 记录用户充值
			...
			nuistRechargeHistoryModel.insertRechargeHistory( nuistLogin.getAccountId(), Double.parseDouble( rechargeModel.getAmount() ),
					request.getRemoteAddr(), RegexUtil.explorerFilter( request.getHeader( "User-Agent" ).toString() ) );
	...
	public List<NameValuePair> getNuistLoginPostParams( NuistLoginModel nuistLoginModel ) {
		...
		postParams.add( new BasicNameValuePair( "NextUrl", "aHR0cDovL3VjYXJkLm51aXN0LmVkdS5jbjo4MDcwL1N5bkNhcmQvTWFuYWdlL1RyYW5zZmVy" ) );
		return postParams;
	}
	...
	public JSONObject chargePWD( String userNumber, ChangePWDModel changePWDModel ) {
		...
		// 修改查询密码
		return NuistCardAPI.nuistPost4JSON( userNumber, NuistCardAPI.NUIST_CARD_CHANGEPWD_API, getNuistChangePWDPostParams( changePWDModel ) );
	}
	...
	public JSONObject cardLost( String userNumber, String Password ) {
		...
		// 挂失请求
		return NuistCardAPI.nuistPost4JSON( userNumber, NuistCardAPI.NUIST_CARD_LOST_API, getNuistCardLostPostParams( Password ) );
	}
	...
	public JSONObject cardUnlost( String userNumber, CardUnlostModel cardUnlostModel ) {
		...
		// 解除挂失请求
		return NuistCardAPI.nuistPost4JSON( userNumber, NuistCardAPI.NUIST_CARD_UNLOST_API, getNuistCardUnlostPostParams( cardUnlostModel ) );
	}
	 ...
	 * @param suggest
	 */
	// public void insertSuggest(String userNumber, NuistSuggestModel
	// nuistSuggestModel) {
	//
	...
	public String postSuggestContent( NuistSuggestModel nuistSuggestModel, String userNumber ) {
		...
		nuistSuggestModel.setSuggest( RegexUtil.scriptTagFilter( nuistSuggestModel.getSuggest() ) );
		nuistSuggestModel.insert();
		return "true";
	}
...
//		StatisticData.centerPopularityRank = nuistTradeModel.query4CenterPopularityRank();
	...
	private void insertNuistTradeByList( List<NuistLogin> list ) {
		...
		while( !list.isEmpty() ) {
			...
			if( NuistCardAPI.nuistPostJSON4RequesrtState( userNumber, NuistCardAPI.NUIST_CARD_LOGIN_API, getNuistLoginPostParams( model ) ) ) {
				...
				case 1:
					...
					do {
						...
						try {
							Elements tbs = NuistCardAPI.getNuistTable( userNumber, "?beginTime=" + year + "/01/01&endTime=" + year + "/12/31" );
							NuistTradeModel.insertNuistTradeByTables( tbs, userNumber, model.getAccountId(), nuist.getNuistTradeModel()
									.acquiredTargetRepository(), nuist.getNuistShopModel().acquiredTargetRepository() );
						} catch( Exception e ) {
							continue;
						}
						index++;
				...
				case 2:
					try {
						...
						NuistTradeModel.insertNuistTradeByTables( tbs, userNumber, model.getAccountId(), nuist.getNuistTradeModel().acquiredTargetRepository(),
		...
		while( !list.isEmpty() ) {
			...
			if( moblieLogin( model ) ) {
				...
				case 1:
					do {
						try {
							JSONObject jsonObject = NuistCardAPI.nuistPost4JSON( model.getUsernumber(), NuistCardAPI.NUIST_CARD_MOBILE_SCORE,
									getNuistMobileScorePostParams( model.getObj(), String.valueOf( i ) ) );
							hasScore = jsonObject.getBoolean( NuistCardAPI.REQUEST_STATE );

							if( hasScore ) {
								i++;
								insertScores( ( long )model.getAccountId(), jsonObject.getJSONArray( NuistCardAPI.REQUEST_OBJ ) );
							}
	...
	public String postChatContent( NuistChatModel nuistChatModel, String userNumber, String ip ) {
		...
		if( null != accountId ) {
			...
			nuistChatModel.setContent( RegexUtil.scriptTagFilter( nuistChatModel.getContent() ) );
