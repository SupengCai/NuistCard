...
public class NuistController {
	...
	public String chat( HttpServletRequest request, HttpServletResponse response, Model model ) {
		...
		if( null != cookie ) {
			String userNumber = cookie.getValue();
			nuistServiceImpl.setUserInfoModel( userNumber, model );
			nuistServiceImpl.fillExplorerAndDevice( model, request.getHeader( "User-Agent" ).toString() );
	...
	public String chat( NuistChatModel nuistChatModel, HttpServletRequest request, HttpServletResponse response ) throws UnsupportedEncodingException {

		String result = "";
		Cookie cookie = CookieTool.getCookieByName( request, CookieTool.COOKIE_USER_NUMBER );
		if( null != cookie )
			result = nuistServiceImpl.postChatContent( nuistChatModel, cookie.getValue(), request.getRemoteAddr() );
